...
func TestMarkOrderAsPaidUseCase_Execute(t *testing.T) {
	t.Run("order is pending", func(t *testing.T) {
		...
		uc := application.NewMarkOrderAsPaidUseCase(repo)
	...
	t.Run("order cannot transition to paid", func(t *testing.T) {
		repo := memory.NewOrderRepository()
		order := makeOrder(t)
